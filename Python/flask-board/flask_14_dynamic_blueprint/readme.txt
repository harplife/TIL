...
-- blueprint --
참고: blueprint자체가 어려운건 아니지만 blueprint하기 위해
기능별로 패키지 생성하고 코드 옮기고, 코드 수정하는게 약간 복잡해진다.

1. 기능(user, posts, main)별로 폴더 생성
   - users => 계정 생성, 업데이트, 비번 설정 관련 기능들을 여기에 모은다.
   - posts => 게시물 올리기, 업데이트, 지우기 관련 기능들을 여기에 모은다.
   - main => home, about
2. 위에 기능별로 routes.py를 각 폴더에 생성하고, 기존 routes.py로 부터
   기능 관련 route들을 각 기능 routes.py에 넣은다 (기존 routes.py에선 지우기).
   - 각 routes.py에서 blueprint 임포트하고 인스턴스를 생성한다 (코드 참고).
   - 각 routes.py에서 @app.route를 @기능.route로 바꿔준다 (코드 참고).
3. 각 기능 폴더에 __init__.py를 생성한다 (비어있어도 된다).
4. 위에 routes.py 했던것 처럼 forms.py도 똑같은 작업을 한다.
5. import문들도 제대로 챙겨줘야 된다~
6. 각 template에서 url_for() 부분을 기능별로 바꿔준다;
   - 예) url_for(home) => url_for(main.home)
7. 최상위 폴더에 있는 __init__.py에 create_app 함수 참고;


-- config --
1. config.py를 생성하고 __init__.py에서 기존에 사용했던 config 부분들을 옮긴다.
   - Config란 클래스 생성, 코드 추가 (코드 참고)
2. __init__.py에서 Config 객체를 불러오는 작업을 한다 (코드 참고)


-- 에러 제어 --
1. 위에 blueprint 기능을 사용한다.
2. errors 패키지 생성 -> handlers.py 생성 -> 코드 참고
3. __init__.py에서 errors blueprint 불러온다.


-- 앱 deployment --
1. 필요 library들을 설치해 준다. (만약 밑에 코맨드가 안되면 하나씩.. pip)
   - pip install -r requirements.txt
2. localhost에서 웹을 돌릴때;
   - python run.py
3. 내부 인터넷망에 웹을 돌릴때;
